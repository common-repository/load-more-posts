...
* Plugin URI:
* Description: This Plugin add to Load More Posts option. Use shortcode (see admin setting...) to display all posts for any page.
* Version: 1.4.0
* Author: Rajeev Chauhan
* Author URI: mailto:chauhanrajeev85@gmail.com
* Text Domain: loadmorepost
* Domain Path: /languages
* Requires at least: 4.9
* Tested up to: 6.6
* Requires PHP: 6.2.4
* @package loadmorepost 
* @category Wordpress
...
if ( ! class_exists( 'loadmorepost' ) ) :
class loadmorepost 
{

	function __construct()
	{
	  register_activation_hook( __FILE__,array(&$this, 'loadmorepost_activate_pliugin' ));   
	...
	function wp_add_css_and_scripts() {
		wp_register_style( 'wp_loadmore_css', plugins_url('css/custom.css',__FILE__ ),array(), "1.3.3");
		wp_enqueue_style( 'wp_loadmore_css' );
		wp_register_script( 'wp_loadmore_js', plugins_url('js/custom.js',__FILE__ ), array( 'jquery' ), "1.2.2");
	...
	function loadmorepost_localization_init_textdomain()
	  ...
	  function loadmorepost_activate_pliugin()
	...
	$html .='<div class="wp_all_posts_section">';
		...
		foreach($results as $result){			
            ...
            $excerpt_content_lmt = get_option('wp_post_excerpt_content','120');
            $the_excerpt = get_the_excerpt($post_id);
            if(!empty($the_excerpt)){
            	 $content = substr($the_excerpt, 0, $excerpt_content_lmt)."...";
            }else{
            	 $content = substr($result->post_content, 0, $excerpt_content_lmt)."...";
            }
